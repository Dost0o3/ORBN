...
export async function canAccessObject({
  ...
  if (!aclPolicy) {
    // Backwards-compat: legacy uploads (made before the finalize endpoint
    // existed) have no ACL metadata. We still want them readable by any
    // authenticated user so we don't break existing avatars/posts. Anonymous
    // callers are still rejected.
    return Boolean(userId) && requestedPermission === ObjectPermission.READ;
  }
