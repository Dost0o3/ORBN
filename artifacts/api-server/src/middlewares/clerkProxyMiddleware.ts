...
export function clerkProxyMiddleware(): RequestHandler {
  // Replit uses a live Clerk instance (pk_live_) even in development,
  // so the proxy must run in all environments, not just production.
  ...
  return createProxyMiddleware({
    ...
    on: {
      proxyReq: (proxyReq, req) => {
        ...
        // Native mobile clients send Origin like "ift-mobile://" which Clerk rejects
        // ("origin_invalid"). Rewrite Origin to match the proxy host so Clerk accepts it.
        proxyReq.setHeader("Origin", `${protocol}://${host}`);
        proxyReq.setHeader("Referer", `${protocol}://${host}/`);

        // Diagnostic: log sign_ins requests so we can see what mobile sends
        if (req.url?.includes("/v1/client/sign_ins") && req.method === "POST") {
          console.log("[clerk-proxy] sign_ins request", {
            method: req.method,
            url: req.url,
