...
export default function SignUpScreen() {
  ...
  const handleSsoSignUp = async (strategy: SsoStrategy) => {
    if (!isLoaded || !signInLoaded || ssoLoading || loading) return;
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      const e = err as { errors?: { message?: string }[] };
      setError(e.errors?.[0]?.message ?? "Verification failed. Check your code.");
    } finally {
      setLoading(false);
    }
  };

  const canSignUp = email.trim().length > 0 && password.length >= 8 && !loading && !ssoLoading;
      ...
      >
        ...
        >
          ...
          {!pendingVerification ? (
            <>
              ...
              {SSO_PROVIDERS.map((p) => {
                ...
                return (
                  <TouchableOpacity
                    key={p.strategy}
                    onPress={() => handleSsoSignUp(p.strategy)}
                    disabled={anyBusy}
                    activeOpacity={0.82}
            ...
            <>
              <Text style={[styles.heading, { color: colors.foreground }]}>Verify email</Text>
              <Text style={[styles.subheading, { color: colors.mutedForeground }]}>
